...
# Build the exact protocol-5 images consumed by the rgpu CLI defaults.  The
...
docker tag \
  ...
  remote-gpu-lupine-client:nccl-rpc-v5

docker build \
  --build-arg SHIM_IMAGE=remote-gpu-lupine-client:nccl-rpc-v5 \
  --tag remote-gpu-hostwide-sandbox:nccl-rpc-v5 \
  --file "$project_root/rgpu-client/containers/hostwide-sandbox/Dockerfile" \
  "$project_root"

docker build \
  --build-arg BASE_IMAGE=remote-gpu-hostwide-sandbox:nccl-rpc-v5 \
  --tag remote-gpu-lupine-pytorch:nccl-rpc-v5 \
...
docker build \
  --build-arg UBUNTU_VERSION=22.04 \
  --target server \
  --tag remote-gpu-lupine-server:nccl-rpc-v5 \
  --file "$lupine_source/Dockerfile" \
  "$lupine_source"

docker build \
  --build-arg SERVER_IMAGE=remote-gpu-lupine-server:nccl-rpc-v5 \
  --tag remote-gpu-lupine-server:nccl-rpc-pytorch-v5 \
  --file "$project_root/rgpu-host/containers/server-overlay/Dockerfile" \
  "$project_root"

docker tag \
  remote-gpu-lupine-server:nccl-rpc-pytorch-v5 \
  "remote-gpu-host:$release_version"

docker tag \
  remote-gpu-lupine-pytorch:nccl-rpc-v5 \
  "remote-gpu-workload:$release_version"

docker build \
  --build-arg SHIM_IMAGE=remote-gpu-lupine-client:nccl-rpc-v5 \
  --build-arg RUNTIME_IMAGE=remote-gpu-lupine-pytorch:nccl-rpc-v5 \
