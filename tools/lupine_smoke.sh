...
config_file=${1:-$project_root/config/hosts.json}
...
python3 "$project_root/tools/upstreams.py" audit lupine
...
docker build \
  ...
  --tag "$client_image" \
  --file "$project_root/containers/common-pytorch/Dockerfile" \
  "$project_root"
...
python3 "$project_root/tools/run_timed.py" \
  ...
  docker run --rm --network host \
    ...
    "$client_image" \
    python3 /workspace/tests/smoke/pytorch_smoke.py \
      --compile \
      --graphs \
      --output /results/lupine-remote-smoke.smoke.json
